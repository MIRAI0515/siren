...
class BatchLinear(nn.Linear, MetaModule):
    '''ハイパーネットワークによって出力される重み行列とバイアスをバッチ処理できる線形メタレイヤー。'''
...
class Sine(nn.Module):
    ...
    def forward(self, input):
        # 論文のセクション3.2の最後の段落および補足資料のセクション1.5で述べられている30という因子について
        return torch.sin(self.omega * input) 


class FCBlock(MetaModule):
    '''ハイパーネットワークと併用する場合に、重みを交換できる全結合ニューラルネットワーク。
    通常のニューラルネットワークとしても使用できます。
    '''
    ...
    def __init__(self, in_features, out_features, num_hidden_layers, hidden_features,
        ...
        # 非線形関数の名前をそれぞれの関数、初期化、及び特定の初期層の初期化方式にマッピングする辞書
        ...
        if weight_init is not None:  # 重みの初期化が渡された場合はそれを上書き
        ...
        if first_layer_init is not None: # 適用可能な場合は特定の初期層の初期化を適用
...
class SingleBVPNet(MetaModule):
    '''境界値問題（BVP）のための標準表現ネットワーク。'''
    # 初期化
    # out_features: 出力層のユニット数
    # type: 活性化関数のタイプ（例: 'sine'）
    # in_features: 入力層のユニット数
    # mode: ネットワークのモード（'mlp', 'rbf', 'nerf' など）
    # hidden_features: 隠れ層のユニット数
    # num_hidden_layers: 隠れ層の数
    # kwargs: その他のオプション引数
    # クラス属性の初期化
    SAVE_DIR_LAYER = None
    HIDDEN_LAYER = 3  # デフォルト値
    
    @classmethod
    def process_root_path(cls, root_path):
        cls.SAVE_DIR_LAYER = root_path
        print(f"Processing root path: {root_path}")

    @classmethod
    def set_hidden_layer(cls, num_hidden_layers):
        """
        HIDDEN_LAYER をクラス属性として設定
        """
        cls.HIDDEN_LAYER = num_hidden_layers
        print(f"HIDDEN_LAYER is set to: {cls.HIDDEN_LAYER}")       
        
    def __init__(self, out_features=1, type='sine', in_features=2,
                 mode='mlp', hidden_features=256, num_hidden_layers=None, **kwargs):
        super().__init__()
        
        # モードによる初期化
        self.mode = mode
        self.num_hidden_layers = self.HIDDEN_LAYER  
        
        # rbf モードの場合、RBF 層を初期化し、in_features を更新
        # nerf モードの場合、Positional Encoding を初期化し、in_features を更新
        if self.mode == 'rbf':
        ...
        # 全結合ブロックの初期化
        self.net = FCBlock(in_features=in_features, out_features=out_features, num_hidden_layers=self.num_hidden_layers,
                           hidden_features=hidden_features, outermost_linear=True, nonlinearity=type)
        
        # 保存フラグとディレクトリ初期化
        self.save_dir = self.SAVE_DIR_LAYER
        self.save_output = False
        
        # ネットワーク構成の出力
        #print(self)
        print('Network configuration: {} '.format(self))
        
    def enable_saving(self, save_dir=None):
        """
        層出力保存を有効化し、保存先ディレクトリを設定
        """
        self.save_output = True
        if save_dir is not None:
            self.save_dir = save_dir
        elif self.SAVE_DIR_LAYER is not None:
            self.save_dir = self.SAVE_DIR_LAYER
        else:
            raise ValueError("SAVE_DIR_LAYER is not set and save_dir is not provided.")
    ...
    """
    def enable_saving(self, save_dir=SAVE_DIR_LAYER):
        #層出力保存を有効化し、保存先ディレクトリを設定
        self.save_output = True
        self.save_dir = save_dir
        os.makedirs(self.save_dir, exist_ok=True)
    ...
    # 追加箇所
    def save_layer_output(self, layer_output, layer_index):
        # 出力テンソルを [batch_size, num_pixels, num_channels] の形で保存する
        output_dir = os.path.join(self.save_dir, f"layer_{layer_index}") 
    ...
    # フォワードパス
    def forward(self, model_input, params=None, save_output=False):#def forward(self, model_input, params=None):
        if params is None:
            params = OrderedDict(self.named_parameters())
        ...
        x = coords
        for i, layer in enumerate(self.net.net):  # self.net.netはFCBlock内の層リスト
            x = layer(x)
            intermediate_outputs.append(x)
            if self.save_output:
                self.save_layer_output(x, i + 1)
...
class PINNet(nn.Module):
    '''Raissi et al. 2019により使用されたアーキテクチャ。'''
...
class ImageDownsampling(nn.Module):
    '''ダウンサンプリングぼかしカーネルに従ってu,v平面でサンプルを生成する'''
...
def init_weights_normal(m):# 様々な重み初期化関数
    if type(m) == BatchLinear or type(m) == nn.Linear:
        if hasattr(m, 'weight'):
...
def sine_init(m):
    with torch.no_grad():
        if hasattr(m, 'weight'):
            num_input = m.weight.size(-1)
            # 補足資料のセクション1.5にある30という因子について
            m.weight.uniform_(-np.sqrt(6 / num_input) / OMEGA, np.sqrt(6 / num_input) / OMEGA) # defalt:OMEGA=30
