...
# 指定されたサイズ（sidelen）と次元数（dim）に基づいて、範囲が -1 から 1 の座標グリッドを生成する
def get_mgrid(sidelen, dim=2):
    '''Generates a flattened grid of (x,y,...) coordinates in a range of -1 to 1.'''
    # ここでは、sidelen が整数の場合、それを次元数 dim に基づいてタプルに変換しています。
    # 例えば、sidelen が 64 で dim が 2 の場合、sidelen は (64, 64) になります。
    ...
    # 生成されたグリッドの範囲を -1 から 1 にスケーリングします。
    pixel_coords -= 0.5
    pixel_coords *= 2.
    
    # NumPy 配列 pixel_coords を PyTorch の Tensor に変換し、dim 次元にフラット化して返す
...
class PointCloud(Dataset):
    def __init__(self, pointcloud_path, on_surface_points, keep_aspect_ratio=True):
        ...
        self.normals: array([], shape=(18844, 0), dtype=float64)
...
class Camera(Dataset):
    def __init__(self, downsample_factor=1):
        ...
        #ここをかえる。skimageのクラスのままが一番いい。私のクラスを追加したらいい。コピーして、新しくクラスを作ればよい。
...
#追加箇所（class Camera(Dataset):の類似クラス）
class Collagen(Dataset): #()の中のDatasetは、親。
    def __init__(self,input_path,downsample_factor=1):#__init__()は準備
        super().__init__()
        self.downsample_factor = downsample_factor
        self.img =Image.open(input_path).convert('L')# 変更箇所
        #self.img =Image.open('/mnt/siren/mip_nd2_yasuda/Ex03_No.14_PBS_SHG_ELN_FBN_DAPI_880nm003_MIP/240613_2204_EGFP.tiff').convert('L').resize((512,512))
        #transformはDataLoader（Implicit2DWrapper）により、行われる。
        self.img_channels = 1

        if downsample_factor > 1:
            # size = (int(64 / downsample_factor),) * 2 # 変更箇所 #全体画像ではなく一部画像を学習済みモデルにいれる際に使用
            size = (int(512 / downsample_factor),) * 2
            self.img_downsampled = self.img.resize(size, Image.ANTIALIAS)
    ...
    def __getitem__(self, idx):#__となっている奴は、pythonが元々備えている奴
        if self.downsample_factor > 1:
            return self.img_downsampled
        else:
            return self.img
...
"""
#追加箇所（class Camera(Dataset):の類似クラス）
class Collagen_1024(Dataset): #()の中のDatasetは、親。
    def __init__(self, downsample_factor=1):#__init__()は準備
        super().__init__()
        self.downsample_factor = downsample_factor # downsample_factorという変数。(ex)a=Collagenとすると
        self.img =Image.open('/mnt/siren/mip_nd2_yasuda/Ex03_No.14_PBS_SHG_ELN_FBN_DAPI_880nm003_MIP/240613_2204_EGFP.tiff').convert('L')
        print(self.img.size)
        #transformはDataLoader（Implicit2DWrapper）により、行われる。
        self.img_channels = 1

        if downsample_factor > 1:
            size = (int(1024 / downsample_factor),) * 2
            self.img_downsampled = self.img.resize(size, Image.ANTIALIAS)
...
class Implicit2DWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, compute_diff=None):
        ...
        self.transform = Compose([
            Resize(sidelength),# 画像のリサイズ
            ToTensor(),# テンソルに変換
            Normalize(torch.Tensor([0.5]), torch.Tensor([0.5]))# 正規化
        ])
        ...
        # 元のデータセットを保持します。
        self.dataset = dataset
        # 指定されたsidelengthに基づいて座標のメッシュグリッドを計算します。
    ...
    # 指定されたインデックス（idx）のアイテムを取得します。
    def __getitem__(self, idx):
        # 画像に変換を適用します。
        img = self.transform(self.dataset[idx])

        # compute_diffパラメータに基づいて画像の微分を計算します。
